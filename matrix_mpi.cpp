...
int main(int argc, char **argv)
{
  ...
  MPI_Init(&argc, &argv); // MPI environment is initialized
  MPI_Comm_rank(MPI_COMM_WORLD, &processId); // Each process gets unique ID (rank)
  MPI_Comm_size(MPI_COMM_WORLD, &processCount); // Number of processes in communicator will be assigned to variable -> processCount

  slaveTaskCount = processCount - 1; // Number of slave tasks will be assigned to variable -> slaveTaskCount

 if (processId == 0) { // Root (Master) process
  start_time = MPI_Wtime(); // Start time execution
  ...
  // 	    printf("\n");
  //     }
    rows = N/slaveTaskCount; // Determine number of rows of the Matrix A, that is sent to each slave process
    ...
    stop_time = MPI_Wtime(); // Stop timer
    ...
    // }
    // printf ("\n");
    printf("Main Thread | Time elapsed = %f seconds.\n", stop_time - start_time);
  }

  // Slave Processes 
  if (processId > 0) {

    double local_start_time, local_stop_time;
    ...
    local_stop_time = MPI_Wtime(); 

    printf("Thread %d | Time elapsed = %f seconds.\n", processId, local_stop_time - local_start_time);
  }
